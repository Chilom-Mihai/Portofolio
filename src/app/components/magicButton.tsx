...
const MagicButton = ({
  ...
  return (
    ...
    >
      <span
        className={`${otherClasses} gap-[8px] inline-flex h-full w-full cursor-pointer items-center justify-center rounded-lg bg-slate-950 px-[10px] font-medium text-white backdrop-blur-3xl`}
      >
        {position === "left" && icon}
        {title}
        {position === "right" && icon}
